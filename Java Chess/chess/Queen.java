...
public class Queen extends ReturnPiece {
  ...
  public static boolean isLegalMove(ReturnPiece curr, ReturnPiece next, String des) {
    ...
    if ((currRank == nextRank || currFile == nextFile) || // Moving like a rook
        (Math.abs(currRank - nextRank) == Math.abs(currFile - nextFile)) && // Moving like a bishop
            (currRank != nextRank && currFile != nextFile)) {
      ...
      if (currRank == nextRank) {
        ...
        int rank = currRank + rankDir;
        char file = (char) ((int) currFile + fileDir);
        while (rank != nextRank && file != nextFile) {
          if (Chess.findPiece(PieceFile.valueOf(String.valueOf(file)), rank) != null) {
            return false;
          }
          rank += rankDir;
          file = (char) ((int) file + fileDir);
